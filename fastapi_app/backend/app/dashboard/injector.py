...
class DashboardInjector:
    ...
    def inject(self, save_draft: bool = False, openai_error: Optional[str] = None) -> str:
        """
        ...
        try:
            ...
            # Преобразуем рекомендации к Dict[str, Recommendation] для section_generator.generate
            recommendations_for_sections = {k: v[0] for k, v in ai_recs.items() if v}
            sections = self.section_generator.generate(
                history=history,
                recommendations=recommendations_for_sections
            )
            ...
            # Готовим строку ошибки для передачи в форматтер
            error_str = str(openai_error) if openai_error is not None else (str(self._ai_error) if self._ai_error is not None else "")
