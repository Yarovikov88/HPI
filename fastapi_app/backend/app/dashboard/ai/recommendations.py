    ...
    def _prepare_sphere_context(
        ...
        for report in history[-3:]:
            if sphere in report.scores:
                report_metrics = report.metrics if report.metrics is not None else []
        ...
        try:
            ...
            for step in result["action_steps"]:
                action_steps.append(ActionStep(
                    ...
                    expected_impact=float(step["expected_impact"]),
            ...
            evidence = Evidence(
                ...
                historical_success=float(result["evidence"]["historical_success"])
            )
