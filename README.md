...
## О проекте
HPI — система для анализа и повышения человеческого потенциала. Позволяет вести аналитику по сферам жизни, строить отчёты, получать AI-рекомендации и отслеживать прогресс.
...
## FAQ
- **Как добавить свою сферу?**  Добавьте её в `database/questions.md` и обновите мастер-список сфер.
- **Почему не работают AI-рекомендации?**  Проверьте наличие переменной `OPENAI_API_KEY` и доступ к OpenAI API.
- **Как обновить дашборд?**  Просто запустите `main.py` — дашборд обновится автоматически.
...
Для автоматизации форматирования и проверки кода используются pre-commit хуки:
- Black (форматирование)
- isort (сортировка импортов)
- Flake8 (линтинг)
- mypy (проверка типов)
- Проверка обновления CHANGELOG.md
...
1. Установите pre-commit:
   ...
   ```
2. Установите хуки в git:
   ```bash
   pre-commit install
   ```
3. Проверьте работу хуков вручную (опционально):
   ```bash
   pre-commit run --all-files
   ```

> ⚠️ При попытке коммита без обновления CHANGELOG.md commit будет отклонён.