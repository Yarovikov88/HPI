...
const HpiMetrics = ({ metrics }: { metrics: HpiMetric[] }) => (
  <div className={styles.hpiMetrics}>
    {metrics && Array.isArray(metrics) && metrics.map((metric, index) => (
...
const pairNodes = (
  ...
  if (!workNodes || !Array.isArray(workNodes) || !lifeNodes || !Array.isArray(lifeNodes)) {
    return [];
  }
      ...
      {sortedYears.map((year) => {
        ...
        return (
          <div key={year} className={styles.yearSection}>
            ...
            {pairedRows && Array.isArray(pairedRows) && pairedRows.map(({ workNode, lifeNode }, i) => (
