...
### Исправлено
- Устранена критическая ошибка в `create_draft.py`, из-за которой для каждой сферы создавалось 5 метрик вместо 3. Это приводило к некорректному расчету HPI.
- Исправлена структура файла `database/questions.md`: удалены дублирующиеся блоки с метриками, "PRO" метрики сделаны основными.
- Все исторические данные в отчетах (`reports_draft/` и `reports_final/`) были исправлены и приведены к новому формату с 3 метриками.
- Улучшена система логирования ошибок при проверке доступности OpenAI API.
