...
async def post_basic_answers(
    ...
    try:
        ...
        for ans in answers:
            ...
            # Проверяем, существует ли уже ответ для этой даты
            ...
            existing = await db.execute(
                select(Answer).where(
                    ...
                    Answer.date == func.to_date(ans.date, 'YYYY-MM-DD')
                )
            )
            ...
            if existing_answer:
                ...
                # Создаем новый ответ с явной датой
                ...
                new_answer = Answer(
                    ...
                    date=func.to_date(ans.date, 'YYYY-MM-DD')
                )
                db.add(new_answer)
...
async def get_basic_answers(
    ...
    try:
        ...
        result = await db.execute(
            select(Answer).where(
                Answer.user_id == user_id,
                Answer.date == func.to_date(date, 'YYYY-MM-DD')
            )
        )
